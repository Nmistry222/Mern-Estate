...
function ListingItem({ listings }) {
  return (
    <div className='bg-white shadow-md hover:shadow-lg transition-shadow overflow-hidden rounded w-[320px]'>
      <Link to={`/listing/${listings._id}`} className='flex flex-col h-full'>
        <img
          src={listings.imageUrls[0]}
          alt='listing cover'
          className='sm:h-[160px] w-full object-cover hover:scale-105 transition-transform duration-300'
        />
        <div className='p-3 flex flex-col gap-2 w-full flex-grow'>
          ...
          <p className='text-slate-500 mt-2 font-semibold flex items-center'>
            ${listings.offer ? listings.discountPrice.toLocaleString('en-US') : listings.regularPrice.toLocaleString('en-US')}
            {listings.type === 'rent' && ' / week'}
          </p>
